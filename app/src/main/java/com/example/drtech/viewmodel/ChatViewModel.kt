...
class ChatViewModel : ViewModel() {
    ...
    private fun getChats(dataSnapshot: DataSnapshot, senderId: String, receiverId: String) {
        ...
        for (data in dataSnapshot.children) {
            ...
            val condition1 = dataSnapshot.child(post?.id.toString())
                .child("senderId").value.toString() == senderId && dataSnapshot.child(post?.id.toString())
                .child("receiverId").value.toString() == receiverId
            val condition2 = dataSnapshot.child(post?.id.toString())
                .child("receiverId").value.toString() == senderId && dataSnapshot.child(post?.id.toString())
                .child("senderId").value.toString() == receiverId
